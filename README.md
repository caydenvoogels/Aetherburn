...
The Zeus mesh uses a character-specific vertical adjustment of `-96` units relative to its capsule.
...
The animation set is built from 19 animation-only Mixamo FBXs and four generated root-locked action sequences. The character uses a first-person camera at eye level with no spring-arm lag; the full Zeus body is hidden from its owner and remains visible to other players. Crouching lowers the camera while directional locomotion continues. Sliding uses a shorter committed posture with limited steering and speed decay. The movement implementation remains in `AAetherburnCharacter`.
...
The Showcase also includes Hyper's footstep and outline systems. `BP_Thunderlord` has Hyper's `AC_CH_Footsteps`, required `AC_CH_Extended_Movement_Base`, and Actor Identifier components; its directional walk, run, and crouch clips have left/right Hyper footplant notifies. `BP_AetherburnPlayerController` adds Hyper's `AC_PC_Outliner` component, and `BP_Outliner_World_Actor` supplies the global post-process volume. The native controller applies Hyper's blue primary stencil (1) to blue affiliations and red secondary stencil (3) to red affiliations, including newly spawned actors. C++ registers the tags and the Editor target compiled. PIE should be checked to confirm the corrected footsteps and rendered outlines.
